        ...
        switch(requestCode) {
            ...
            case Constants.REQUEST_ACCOUNT_PICKER:
                if (resultCode == RESULT_OK && data != null &&
                        data.getExtras() != null) {
                    String token = data.getStringExtra(AccountManager.KEY_ACCOUNT_NAME);
                    googleAccountDetails = new Account(token, GoogleAuthUtil.GOOGLE_ACCOUNT_TYPE);
                    fetchAuthToken();
                    ...
                    protected Object doInBackground(Object[] objects) {
                        try {
                            Log.d(TAG,"Requesting token for account: " +
                                    googleAccountDetails.name);
                            /*googleAuthToken = GoogleAuthUtil.getToken(getApplicationContext(),
                                    googleAccountName, Constants.GPHOTOS_SCOPE);*/
                            googleAuthToken = GoogleAuthUtil.getToken(getApplicationContext(), googleAccountDetails, Constants.GPHOTOS_SCOPE);
