    ...
    public ArrayList<GDModel> getPhotos(int currentPage) {
        ...
        if (!serverResponse.isEmpty()) {
            try {
                ...
                Log.d(TAG,"Received " + itemsArray.length() + " items");
                String mimeType;
                ...
                for (int i = 0; i < itemsArray.length(); i++) {
                    ...
                    hasThumbnail = jObj.getBoolean(Constants.JSON_FIELD_HAS_THUMBNAIL);
                    if (null != mimeType && (mimeType.startsWith(Constants.JSON_FIELD_MIME_IMG) || mimeType.startsWith(Constants.JSON_FIELD_MIME_VIDEO))) {
                        GDModel photo = new GDModel();
                        photo.title = jObj.getString(Constants.JSON_FIELD_TITLE);
                        Log.i(TAG, "getPhotos: " + photo.title);
                        if (hasThumbnail) {
